...
import { agents, agentRuns, approvals, auditLogs, deliverables, policies, users, workflowSteps, workflows, workspaceState, type InsertUser } from "../drizzle/schema";
...
export async function createApproval(values: typeof approvals.$inferInsert) { const db = await getDb(); if (!db) return undefined; await db.insert(approvals).values(values); return db.select().from(approvals).where(eq(approvals.approvalKey, values.approvalKey)).limit(1).then((rows) => rows[0]); }
export async function getRunByKey(runKey: string) { const db = await getDb(); if (!db) return undefined; return db.select().from(agentRuns).where(eq(agentRuns.runKey, runKey)).limit(1).then((rows) => rows[0]); }
export async function getRunById(id: number) { const db = await getDb(); if (!db) return undefined; return db.select().from(agentRuns).where(eq(agentRuns.id, id)).limit(1).then((rows) => rows[0]); }
...
export async function listWorkflows() { const db = await getDb(); if (!db) return []; return db.select().from(workflows).orderBy(desc(workflows.updatedAt)); }
export async function listWorkflowSteps(workflowId: number) { const db = await getDb(); if (!db) return []; return db.select().from(workflowSteps).where(eq(workflowSteps.workflowId, workflowId)).orderBy(workflowSteps.position); }
export async function createWorkflow(values: typeof workflows.$inferInsert, steps: Array<Omit<typeof workflowSteps.$inferInsert, "workflowId">>) { const db = await getDb(); if (!db) return undefined; await db.insert(workflows).values(values); const workflow = await db.select().from(workflows).where(eq(workflows.workflowKey, values.workflowKey)).limit(1).then((rows) => rows[0]); if (workflow && steps.length) await db.insert(workflowSteps).values(steps.map((step) => ({ ...step, workflowId: workflow.id }))); return workflow; }
export async function listDeliverables() { const db = await getDb(); if (!db) return []; return db.select({ deliverable: deliverables, agent: agents, run: agentRuns }).from(deliverables).leftJoin(agents, eq(deliverables.agentId, agents.id)).leftJoin(agentRuns, eq(deliverables.runId, agentRuns.id)).orderBy(desc(deliverables.updatedAt)).limit(200); }
export async function createDeliverable(values: typeof deliverables.$inferInsert) { const db = await getDb(); if (!db) return undefined; await db.insert(deliverables).values(values); return db.select().from(deliverables).where(eq(deliverables.deliverableKey, values.deliverableKey)).limit(1).then((rows) => rows[0]); }
